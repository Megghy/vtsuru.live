...
- 全局页面（所有主播可用）：`src/views/contrib/pages/<pageId>.vue`
- 主播专属页面（仅该主播可用）：`src/views/contrib/pages/<streamerId>/<pageId>.vue`（`streamerId` 为数字）
...
推荐做法：提交 `Contrib Page` 到 `src/views/contrib/pages/<streamerId>/<pageId>.vue`，并在 `PageMeta` 中标记 `scope: 'streamer'` 与 `streamerId`。
...
如果你明确要改的是“主页模板体系”（`src/views/view/indexTemplate/*`），请说明原因（例如：要替换默认主页视觉/结构），并避免把“子页面”放进该目录。
