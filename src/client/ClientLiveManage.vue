...
import { startLive, stopLive, updateRoom, getLiveAreas, getLiveVersion, updateRoomNews, uploadCover, updateCover, type LiveArea } from './api/live-manage'
...
const checkClientVersion = async () => {
  try {
    ...
    // 检查版本是否大于等于 0.1.6
    const minVersion = '0.1.6'
    // isVersionSupported.value = compareVersion(version, minVersion) >= 0
    
    // if (!isVersionSupported.value) {
    //   console.warn(`客户端版本过低: ${version}, 需要 ${minVersion} 或更高版本`)
    // }
  }
  catch (err) {
