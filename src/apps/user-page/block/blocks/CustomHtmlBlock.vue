...
const iframeHeight = computed(() => config.value.heightMode === 'fixed'
  ? config.value.height
...
function readTheme() {
  ...
  const read = (primary: string, fallback: string) => style.getPropertyValue(primary).trim() || style.getPropertyValue(fallback).trim()
...
watch(() => documentResult.value.srcdoc, () => {
...
<template>
  <BlockCard :framed="config.framed" :backgrounded="config.backgrounded" :content-style="{ padding: 0, overflow: 'hidden' }">
    <div ref="host" class="custom-html-host">
      <iframe
        v-if="themeReady"
        ref="frame"
