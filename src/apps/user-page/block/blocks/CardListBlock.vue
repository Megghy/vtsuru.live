...
const items = computed(() => Array.isArray(values.value.items) ? values.value.items.flatMap((item) => {
  if (!item || typeof item !== 'object' || Array.isArray(item)) return []
  const card = item as Record<string, any>
  const file = card.imageFile && typeof card.imageFile === 'object' && !Array.isArray(card.imageFile) ? card.imageFile : null
  return [{
    ...
    tags: Array.isArray(card.tags) ? card.tags.filter((tag: unknown): tag is string => typeof tag === 'string' && tag.trim().length > 0).slice(0, 8) : [],
...
<template>
  <BlockCard :framed="values.framed !== false" :backgrounded="values.backgrounded !== false">
    <div class="cards" :class="`cards--${layout}`" :style="{ '--card-columns': String(columns) }">
      <article v-for="item in items" :key="item.id" class="card-item">
        <img v-if="item.image" :src="item.image" :alt="item.title" loading="lazy" decoding="async" referrerpolicy="no-referrer" class="card-image">
        <div class="card-content">
          ...
          <div v-if="item.tags.length" class="card-tags">
            <NTag v-for="tag in item.tags" :key="tag" size="small" :bordered="false">
              {{ tag }}
            </NTag>
          </div>
          <div class="card-actions">
            <CardListAction :action="item.primaryAction" :user-name="props.userInfo?.name" />
            <CardListAction :action="item.secondaryAction" :user-name="props.userInfo?.name" secondary />
          </div>
        </div>
      </article>
    </div>
...
.card-content p { color: var(--vtsuru-fg-muted); font-size: 13px; line-height: 1.55; white-space: pre-wrap; }
.card-tags, .card-actions { display: flex; gap: 6px; flex-wrap: wrap; }
.cards--list .card-item { display: grid; grid-template-columns: minmax(140px, 28%) minmax(0, 1fr); }
.cards--list .card-image { height: 100%; min-height: 150px; aspect-ratio: auto; }

@container (max-width: 620px) {
  .cards--grid { grid-template-columns: 1fr; }
  .cards--list .card-item { grid-template-columns: 1fr; }
  .cards--list .card-image { min-height: 0; aspect-ratio: 16 / 9; }
}
</style>
