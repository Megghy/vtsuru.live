...
describe('embed adapters', () => {
  it('把 bilibili 与 YouTube 地址转换为固定播放器地址', () => {
    expect(parseEmbedUrl('https://www.bilibili.com/video/BV1xx411c7mD').src)
      .toBe('https://player.bilibili.com/player.html?autoplay=0&bvid=BV1xx411c7mD')
    expect(parseEmbedUrl('https://youtu.be/dQw4w9WgXcQ').src)
  ...
  it('只允许登记的反馈表单来源', () => {
    ...
    expect(() => parseFeedbackEmbedUrl('https://docs.google.com/forms/d/e/form-id/viewform?redirect=https://evil.test')).toThrow('不支持查询参数')
  })

  it('音乐地址由 provider 适配器生成且 custom 为严格白名单', () => {
    expect(parseMusicEmbedUrl('spotify', 'https://open.spotify.com/track/abc123').src)
      .toBe('https://open.spotify.com/embed/track/abc123')
    expect(parseMusicEmbedUrl('custom', 'https://w.soundcloud.com/player/?url=https%3A%2F%2Fapi.soundcloud.com%2Ftracks%2F1').provider)
      .toBe('soundcloud')
    expect(parseMusicEmbedUrl('netease', 'https://music.163.com/outchain/player?type=2&id=123').src)
      .toContain('type=2&id=123')
    expect(() => parseMusicEmbedUrl('custom', 'https://w.soundcloud.com/player/?url=https%3A%2F%2Fevil.test%2Ftrack')).toThrow('播放内容域名不受支持')
    expect(() => parseMusicEmbedUrl('custom', 'https://w.soundcloud.com/player/?url=https%3A%2F%2Fsoundcloud.com%2Fa&redirect=https%3A%2F%2Fevil.test')).toThrow('不支持查询参数')
