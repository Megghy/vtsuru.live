...
describe('validateBlockPageProject', () => {
  it('校验所有层级的区块 ID 全局唯一', () => {
    ...
    expect(result.ok === false ? result.issues : []).toContainEqual(expect.objectContaining({
      blockId: 'same',
      fieldPath: 'id',
  ...
  it('二维码区块校验样式和内容容量', () => {
    expect(validateBlockPageProject(project([{
      id: 'qrcode',
      type: 'qrcode',
      props: {
  ...
  it('不校验当前模式不会使用的属性', () => {
    expect(validateBlockPageProject(project([
      { id: 'row-videos', type: 'videoList', props: { source: 'userIndex', layout: 'row', columns: 99, items: [{ url: 'invalid' }] } },
      { id: 'question-box', type: 'feedback', props: { embed: true, embedMode: 'questionBox', url: 'invalid', height: 1 } },
    ]))).toMatchObject({ ok: true })
  })
  ...
  it('校验功能入口键值和唯一性', () => {
    ...
    expect(result.ok === false ? result.issues.map(issue => issue.fieldPath) : []).toEqual(expect.arrayContaining([
      'items[1].key',
      'items[2].key',
    ]))
  })
  ...
  it('校验歌单区块样式和数量', () => {
    ...
    expect(result.ok === false ? result.issues.map(issue => issue.fieldPath) : []).toEqual(expect.arrayContaining(['variant', 'maxItems']))
  })
  ...
  it('校验图文卡片及按钮目标', () => {
    ...
    const result = validateBlockPageProject(project([{
      id: 'bad-cards',
      type: 'cardList',
      props: { layout: 'grid', columns: 8, items: [{ tags: [1], primaryAction: { label: '查看', url: 'http://unsafe.test' } }] },
    }]))
    expect(result.ok === false ? result.issues.map(issue => issue.fieldPath) : []).toEqual(expect.arrayContaining([
      'columns',
      'items[0].tags',
      'items[0].primaryAction.url',
    ]))
  })
  ...
  it('校验精选积分商品配置', () => {
    ...
    expect(result.ok === false ? result.issues.map(issue => issue.fieldPath) : []).toEqual(expect.arrayContaining(['count', 'selection']))
  })
  ...
  it('校验自动页面目录配置', () => {
    ...
    expect(result.ok === false ? result.issues.map(issue => issue.fieldPath) : []).toEqual(expect.arrayContaining(['layout', 'levels']))
  })
