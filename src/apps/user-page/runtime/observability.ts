...
export type UserPageErrorPhase =
  | 'user'
  | 'settings'
  | 'bili-profile'
  | 'render'
...
export function reportPublicPageError(
  cause: unknown,
  phase: Extract<UserPageErrorPhase, 'user' | 'settings' | 'bili-profile' | 'render'>,
) {
  reportError(cause, phase, 'public-user-page')
}

export function reportUserPageError(
  cause: unknown,
  phase: Extract<
    UserPageErrorPhase,
    'save-draft' | 'clear-draft' | 'discard-local-changes' | 'publish' | 'rollback' | 'upload'
  >,
) {
