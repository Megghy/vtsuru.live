...
describe('draft preview token', () => {
  ...
  it('只允许同源存储中的令牌读取一次', () => {
    const settings = { version: 1, home: { mode: 'legacy' as const }, pages: {} } as const
  ...
  it('拒绝过期、跨用户或格式非法的令牌', () => {
    const crossUserToken = createDraftPreview(42, { version: 1, pages: {} })
    expect(consumeDraftPreview(crossUserToken, 43)).toBeNull()

    const token = createDraftPreview(42, { version: 1, pages: {} })
