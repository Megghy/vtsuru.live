  ...
  <NForm label-placement="top" size="small">
    ...
    <PropsGrid :col-gap="8" :row-gap="0">
      <NFormItem label="布局类型">
        <NSelect
          v-model:value="layout.layout" :options="[
      ...
      <NFormItem v-if="layout.layout === 'row'" label="允许换行">
        <NFlex justify="end">
          <NSwitch v-model:value="layout.wrap" size="small" />
        </NFlex>
      </NFormItem>
      <NFormItem v-if="layout.layout === 'grid'" label="列数">
        <NInputNumber v-model:value="layout.columns" :min="1" :max="12" style="width: 100%" />
      </NFormItem>
      <NFormItem label="间距">
        <NInputNumber v-model:value="layout.gap" :min="0" :max="80" style="width: 100%" />
      </NFormItem>
      <NFormItem label="最大宽度">
        <NInput v-model:value="layout.maxWidth" placeholder="如 100% / 480px" />
      </NFormItem>
      <NFormItem label="横向对齐">
        <NSelect v-model:value="horizontalModel" :options="horizontalOptions" />
      </NFormItem>
      <NFormItem label="纵向对齐">
        <NSelect v-model:value="verticalModel" :options="verticalOptions" />
      </NFormItem>
    </PropsGrid>
    <NText depth="3" style="display: block; margin-top: 12px; padding: 6px 8px; background: var(--vtsuru-bg-muted); border-radius: 4px; font-size: 12px; line-height: 1.5">
