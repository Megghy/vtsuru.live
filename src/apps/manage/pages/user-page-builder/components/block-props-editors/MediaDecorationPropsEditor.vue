...
import { NAlert, NButton, NDatePicker, NFlex, NForm, NFormItem, NIcon, NInput, NInputNumber, NSelect, NSwitch, NText } from 'naive-ui'
...
<template>
  <NForm v-if="props.block.type === 'quote'" label-placement="top" size="small">
    <PropsGrid>
      <NFormItem class="span-full" label="内容">
        <NInput v-model:value="blockProps.text" type="textarea" :autosize="{ minRows: 3, maxRows: 8 }" />
      </NFormItem>
      <NFormItem label="作者">
      ...
      <NFormItem label="对齐">
        <NSelect
          v-model:value="blockProps.align" :options="[
  ...
  <NForm v-else-if="props.block.type === 'marquee'" label-placement="top" size="small">
    <PropsGrid>
      ...
      <NFormItem label="悬停暂停">
        <NFlex justify="end">
          <NSwitch v-model:value="blockProps.pauseOnHover" size="small" />
        </NFlex>
      </NFormItem>
    </PropsGrid>
  </NForm>

  <NForm v-else-if="props.block.type === 'countdown'" label-placement="top" size="small">
    <PropsGrid>
      <NFormItem class="span-full" label="标题">
        <NInput v-model:value="blockProps.title" placeholder="例如：生日倒计时" />
      </NFormItem>
      <NFormItem class="span-full" label="目标时间">
        <NDatePicker v-model:value="countdownTarget" type="datetime" clearable placeholder="选择目标时间" style="width: 100%" />
      </NFormItem>
      <NFormItem label="展示样式">
        <NSelect v-model:value="blockProps.style" :options="[{ label: 'cards', value: 'cards' }, { label: 'inline', value: 'inline' }]" />
      </NFormItem>
      <NFormItem label="显示秒">
        <NFlex justify="end">
          <NSwitch v-model:value="blockProps.showSeconds" size="small" />
        </NFlex>
      </NFormItem>
      <NFormItem class="span-full" label="到达后文案">
        <NInput v-model:value="blockProps.doneText" placeholder="已到达" />
      </NFormItem>
    </PropsGrid>
  </NForm>
  ...
  <NForm v-else-if="props.block.type === 'feedback'" label-placement="top" size="small">
    <PropsGrid>
      ...
      <NFormItem label="嵌入到页面">
        <NFlex justify="start">
          <NSwitch v-model:value="blockProps.embed" size="small" @update:value="enableFeedbackEmbed" />
          <NAlert type="info" :show-icon="false" style="font-size: 12px; padding: 6px 10px; border-radius: var(--vtsuru-page-radius)">
            开启“嵌入到页面”后，可选择站内提问箱
          </NAlert>
        </NFlex>
      </NFormItem>
  ...
  <NForm v-else-if="props.block.type === 'image'" label-placement="top" size="small">
    <PropsGrid>
      <NFormItem label="最大宽度">
        <NInput v-model:value="blockProps.maxWidth" placeholder="例如 100% 或 480px" />
      </NFormItem>
      <NFormItem label="最大高度">
        <NInput v-model:value="blockProps.maxHeight" placeholder="例如 100% 或 320px" />
      </NFormItem>
      <NFormItem label="形状">
        <NSelect
          v-model:value="blockProps.shape" :options="[
      ...
      <NFormItem label="图片描述 (Alt)">
        <NInput v-model:value="blockProps.alt" placeholder="图片加载失败时显示的文字" />
      </NFormItem>
      <NFormItem class="span-full" label="本地图片">
        <NFlex align="center">
          <NButton size="small" :loading="editor.isUploading.value" @click="editor.triggerUpload(props.block, 'imageFile')">
            <template #icon>
              <NIcon><ImageOutline /></NIcon>
            </template>
            上传图片
          </NButton>
          <NButton size="small" secondary :disabled="!blockProps.imageFile" @click="editor.clearUploadedFile(props.block, 'imageFile')">
            清除
          </NButton>
          <img
  ...
  <NForm v-else-if="props.block.type === 'embed'" label-placement="top" size="small">
    <PropsGrid>
      <NFormItem class="span-full" label="嵌入链接 (支持 Bilibili / YouTube)">
        <NInput v-model:value="blockProps.url" placeholder="https://www.youtube.com/watch?v=..." />
      </NFormItem>
      <NFormItem label="标题">
        <NInput v-model:value="blockProps.title" placeholder="可选，用于无障碍访问" />
      </NFormItem>
    </PropsGrid>
  </NForm>

  <NForm v-else-if="props.block.type === 'divider'" label-placement="top" size="small">
    <PropsGrid>
      <NFormItem label="文字">
      ...
      <NFormItem label="上边距 px">
        <NInputNumber v-model:value="blockProps.marginTop" :min="0" :max="80" style="width: 100%" />
      </NFormItem>
      <NFormItem label="下边距 px">
        <NInputNumber v-model:value="blockProps.marginBottom" :min="0" :max="80" style="width: 100%" />
      </NFormItem>
    </PropsGrid>
  </NForm>

  <NForm v-else-if="props.block.type === 'spacer'" label-placement="top" size="small">
    <NFormItem label="大小">
      <NSelect v-model:value="blockProps.size" :options="[{ label: 'sm', value: 'sm' }, { label: 'md', value: 'md' }, { label: 'lg', value: 'lg' }]" />
    </NFormItem>
  </NForm>

  <NForm v-else-if="props.block.type === 'footer'" label-placement="top" size="small">
    <NFormItem label="文字">
      <NInput v-model:value="blockProps.text" />
    </NFormItem>
  </NForm>
