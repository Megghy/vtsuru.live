...
<template>
  <PropsGrid>
    <NFormItem :label="`${label}文字`">
      <NInput v-model:value="action.label" placeholder="留空则不显示" />
    </NFormItem>
    ...
    <NFormItem class="span-full" :label="`${label}目标`">
      <NSelect v-if="action.page" v-model:value="action.page" :options="pageOptions" />
      <NInput v-else v-model:value="action.url" placeholder="https://..." />
    </NFormItem>
  </PropsGrid>
</template>
