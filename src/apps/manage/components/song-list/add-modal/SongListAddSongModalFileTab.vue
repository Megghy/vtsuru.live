      ...
      .map((row) => {
        ...
        for (let i = 0; i < headers.length; i++) {
          ...
          if (columnMappings.value.translateName) {
            const translateNameHeaders = columnMappings.value.translateName
              .split(/,|，/)
    ...
    <NCollapse v-if="useCustomColumnMapping">
      ...
      >
        <NFlex vertical>
          ...
          <NAlert type="info"> 设置完成后请点击"保存映射"，设置将自动保存到本地浏览器，下次访问时仍会使用 </NAlert>
        </NFlex>
      </NCollapseItem>
    </NCollapse>
  </NFlex>

  <NDivider> 文件上传 </NDivider>
