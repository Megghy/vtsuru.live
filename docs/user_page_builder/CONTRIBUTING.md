...
涉及公开展示页或 Block viewer 样式时，先阅读 [用户展示页样式与主题接入指南](./PUBLIC_PAGE_STYLE_GUIDE.md)。

## 0. UI/UX 风格（统一：shadcn）

- 视觉：克制、清晰层级、轻边框/轻阴影，避免“大面积高饱和色块 + 厚重阴影”的堆叠。
- 密度：紧凑高效但不拥挤；常用点击热区建议 ≥ 32px，高频操作 ≥ 36px。
- 交互：hover/active/focus 状态必须明显；focus ring 清晰（键盘可用性）。
- 排版：优先单列移动端；文本/按钮不挤压、不溢出；重要信息优先对齐与分组。
- 实现：可继续用 Naive UI 组件，但样式与交互必须对齐 shadcn 风格（token/变量优先，少写“到处散落的硬编码像素”）。
...
- 优先复用现有组件（Naive UI），并按 shadcn 风格实现视觉与交互
...
- 体验
  - 移动端优先：窄屏不崩、不横向溢出
  - 视觉与交互遵循 shadcn 风格（层级清晰、间距合理、状态明显、对比度足够）
- 性能
...
- 代码风格遵循仓库现有写法（Vue 3 + TS），并遵守本指南的 shadcn UI/UX 规范。
